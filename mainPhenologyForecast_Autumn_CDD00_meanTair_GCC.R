  ...
  foreach(i=iseq) %dopar% {
    ...
    for(d in 1:length(dates)){
      ...
      if(!file.exists(outputFile)){
        partialFileName <- paste(saveDirectory,siteName,"_",startDate,"_",endDate,"_CDD_00_meanTair_outBurn_GCC_PARTIAL_FILE.RData",sep="")
        outBurn <- phenologyForecast_Autumn(forecastType = "CDD_meanTair",forecastLength = forecastLength,
                                            ...
                                            season="fall",baseTemp=0, index="GCC",
                                            baseNum=20000,
                                            iterSize=10000,
