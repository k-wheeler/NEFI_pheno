...
pdf(file=paste(siteName,"_PhenologyForecast_accuracy.pdf",sep=""),height=10,width=6)
par(mfrow=c(3,1))
#plot(plotDays,rep(NA,length(plotDays)),ylim=c(1,160),xlim=c(1,160),xlab="Forecast Day",ylab="Transition Estimate")
###Random Walk
plot(plotDays,rep(NA,length(plotDays)),ylim=c(125,160),xlim=c(1,160),
...
###Logistic Covariate
plot(plotDays,rep(NA,length(plotDays)),ylim=c(125,160),xlim=c(1,160),xlab="Forecast Day",ylab="Transition Estimate",main="Logistic Covariate")
ciEnvelope(allPlotDays,rep(tran50[3],length(allPlotDays)),rep(tran50[1],length(allPlotDays)),col="gray")
abline(h=tran50[2],col="black",lwd=2)
abline(v=tran50[2],col="black",lwd=2)
