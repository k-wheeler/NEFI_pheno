...
*Please cite the paper if you use this package: Wheeler, K.I., Dietze, M.C. (in press). "A statistical model for estimating midday NDVI from the Geostationary Operational Environmental Satellite (GOES) 16 and 17." Remote Sensing.*
...
1. The package relies on the file names being consistent: specifically the GOES data file names, the calculated NDVI file names, and the diurnal fit output file name all should not be changed.
1. Currently this won't fully work on days that have nautical dawn before timezone (e.g., 5 for eastern USA) number of hours after midnight (e.g., Won't work calculate/include NDVI values before 5am in eastern USA).
1. Currently, this won't work for the last day of the year yet.
