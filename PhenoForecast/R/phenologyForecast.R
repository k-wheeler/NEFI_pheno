...
phenologyForecast <- function(forecastType,forecastLength=14,siteName,URL,lat,long,dataDirectory,startDate,endDate,cValsPC,dValsPC,cValsMN,dValsMN,cValsME,dValsME,GEFS_Files="",GEFS_Directory){
  ...
  if(forecastType=="randomWalk"){
    ...
    if(forecastType=="logisticCov"){
      datSf <- createSf(lat=lat,long=long,dates=days,siteName=siteName,dataDirectory=dataDirectory,endDate=endDate,GEFS_Files=GEFS_Files,GEFS_Directory=GEFS_Directory)
